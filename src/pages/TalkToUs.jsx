...
//         "https://homserver.onrender.com/talkExpertForm",
//         {
//           method: "POST",
//           headers: {
...
const TalkToUs = () => {
  ...
  const validate = () => {
    ...
    if (!formData.collabObjective) {
      newErrors.collabObjective = "Collaboration Objective is required.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://homserver.onrender.com/talkExpertForm", // Change this to your backend URL
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="flex flex-col lg:flex-row bg-white">
        ...
        <div className="bg-yellow-50 rounded-lg p-8 shadow-lg w-full lg:w-1/2 mt-8 pt-32 px-20 lg:mt-0">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <textarea
              name="collabObjective"
              placeholder="Collaboration Objective*"
              required
              value={formData.collabObjective}
              onChange={handleChange}
              className={`w-full p-3 mb-4 border ${
                errors.collabObjective ? "border-red-500" : "border-gray-300"
              } rounded-md focus:ring focus:ring-blue-200`}
            />
            {errors.collabObjective && (
              <p className="text-red-500 text-sm">{errors.collabObjective}</p>
            )}

            <button
              type="submit"
              className="w-full p-3 bg-black text-white rounded-md hover:bg-gray-700 mt-4"
            >
              Submit
            </button>
          </form>
